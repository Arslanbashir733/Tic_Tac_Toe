...
public class MultiPlayerActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    //An int based on the numbers of Button in each row ord coloum
    private int dimention = 4;
    //Make a newButton array to take a 4 x 4 amount of Button
    private android.widget.Button[][] Button = new Button[dimention][dimention];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Get the id of eachButton on the board by counting 4 x 4 and saving them theButton array
       ...
       Button back = findViewById(R.id.back);
        back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                startActivity(new Intent(MultiPlayerActivity.this, MultiPlayerLevelsActivity.class));
            }
        });
    }
    ...
    public void onClick(View view) {
        ...
        //Check if the Button are not empty and continue if they are
        if (!((Button) view).getText().toString().equals("")){
            return;
        }

        //If it's player 1 turn and theButton is clicked, then change the text on theButton to X
        if (p1Turn){
            ((Button)view).setText("X");
        }
        //If it's not player 1 turn and theButton is clicket, then change the text on theButton to O
        else {
        ...
        if (checkForWin()){
            ...
            //If the statment returned is true and the amount of rounds equals the amount of the Button, then the pause for 1 second and run the draw function
            Handler handler = new Handler();
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
    ...
    private boolean checkForWin(){
        ...
        //Set the text from all the Button in the new string array so they can changed afterwards
        for (int i = 0; i < dimention; i++){
            for (int j = 0; j < dimention; j++){
    ...
    private void CleanBoard(){
        //Clean the text on everyButton and reset the rounds, and set player 1 turn to true
        for (int i=0; i<dimention; i++){
            for (int j=0;j<dimention;j++){
                Button[i][j].setText("");
            }
